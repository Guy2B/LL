// Lune Dashboard — Gate fix + robust handling
(function(){
  const PASS='lune';
  ...
  function open(){ if(gate) gate.remove(); }
  function fail(){ msg?.classList?.remove('hidden'); }

  function auth(){
    if(!input) return; const v=input.value||'';
    if(v===PASS){ if(remember?.checked) localStorage.setItem('LUNE_DASH_OK','1'); open(); }
    else{ fail(); }
  }
  if(localStorage.getItem('LUNE_DASH_OK')==='1'){ open(); }
  btn?.addEventListener('click', auth);
  input?.addEventListener('keydown', (e)=>{ if(e.key==='Enter'){ e.preventDefault(); auth(); }});
  ...
  // Table + actions (same as earlier build)
  ...
  function render(){ const list=getUsers(); if(!tbody) return; tbody.innerHTML=''; list.forEach((r,i)=>{ const score=r.quickScore || (r.scan&&typeof r.scan.texture==='number'? Math.round(r.scan.texture*100):''); const tr=document.createElement('tr'); tr.innerHTML=`
    <td>${fmtDate(r.timestamp)}</td>
    <td>${esc(r.intake?.name||'')}</td>
    <td>${esc(r.intake?.email||'')}</td>
    <td>${esc(r.payment?.product||'')}</td>
    <td>${esc(score)}</td>
    <td><button class="btn subtle" data-act="prompt" data-i="${i}">Prompt</button>
        <button class="btn" data-act="report" data-i="${i}">Report</button></td>`; tbody.appendChild(tr); }); }
  render();

  tbody?.addEventListener('click',e=>{
    const k=e.target.closest('button'); if(!k) return; const i=+k.getAttribute('data-i'); const act=k.getAttribute('data-act'); const users=getUsers(); selectedIndex=i;
    if(act==='report'){ localStorage.setItem('LUNE_APP_STATE', JSON.stringify(users[i])); window.location.href='../app/report/report.html'; }
    if(act==='prompt'){ openPromptFor(users[i]); }
  });

  function openPromptFor(rec){ document.querySelector('[data-panel="prompt"]').click();
    const set=(id,val)=>{ const el=document.getElementById(id); if(el) el.textContent=val; };
    set('ctxName', rec.intake?.name||'—'); set('ctxPlan', rec.payment?.product||'—'); set('ctxScore', (typeof rec.quickScore==='number'?rec.quickScore:'—'));
    const goals=(rec.intake?.skinGoals||[]).join(', '), allergies=(rec.intake?.allergies||[]).join(', '), type=rec.intake?.skinType||'—', score=rec.quickScore||'';
    const tpl=`Client: ${rec.intake?.name||'—'}\nEmail: ${rec.intake?.email||'—'}\nSkin type: ${type}\nGoals: ${goals}\nAllergies: ${allergies}\nQuick Score: ${score}/100\n\nTask: Draft a friendly follow-up (100–140 words) explaining next steps, respecting pregnancy/allergies, and when to book a re-scan.`;
    const pt=document.getElementById('promptText'); if(pt) pt.value=tpl;
    const rj=document.getElementById('resultsJson'); if(rj) rj.value=JSON.stringify(rec,null,2);
  }
