...
(() => {
  ...
  function addCloudPanel() {
    ...
    panel.innerHTML = `
      <h2>Connexion Cloud Staff</h2>
      <p class="small">URL Apps Script + token privé. Une fois connecté, l'app synchronise automatiquement RDV, clients, factures, paiements, stock et paramètres.</p>
      <div class="row">
      ...
      <details class="fold cloudAdvancedTools" style="margin-top:10px;">
        <summary>Secours technique</summary>
        <p class="small">À utiliser seulement si un appareil doit être initialisé ou réparé.</p>
        <div class="backupGrid noPrint" style="display:flex;flex-wrap:wrap;gap:8px;">
  ...
  async function pullCloud(manual = false) {
    try {
      ...
      emitCloudEvent("lune:cloud-db-updated", { action: "pull", manual, version: res.version || "" });
  ...
  async function pushCloud(manual = false) {
    try {
      ...
      emitCloudEvent("lune:cloud-db-updated", { action: "push", manual });
