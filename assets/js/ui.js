...
const luneBookingForm = document.getElementById("booking-form");
if (luneBookingForm) {
  luneBookingForm.addEventListener("submit", function(e) {
    // booking-cloud.js handles the official Cloud + email notification workflow.
    // This fallback is only used if booking-cloud.js is not loaded.
    if (window.LuneBookingCloud && window.LuneBookingCloud.handlesSubmit) return;
    ...
    if (success) {
      success.textContent = "✅ Vielen Dank. Ihre Anfrage wird bearbeitet. Wir melden uns in Kürze zur Bestätigung.";
