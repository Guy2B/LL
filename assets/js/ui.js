...
document.getElementById("booking-form")?.addEventListener("submit", (e) => {
  ...
  const [name, phone, service, date, time] = e.target.elements;

  const message =
    `Termin Anfrage:%0A` +
    `Name: ${name.value}%0A` +
    `Telefon: ${phone.value}%0A` +
    `Service: ${service.value}%0A` +
    `Datum: ${date.value}%0A` +
    `Uhrzeit: ${time.value}`;

  window.open(`https://wa.me/491791544002?text=${message}`, "_blank");
});
document.getElementById("booking-form")?.addEventListener("submit", (e) => {
  e.preventDefault();
  ...
  const message =
    `🌿 Lune Beauty Terminanfrage%0A%0A` +
    `Name: ${name}%0A` +
    `Telefon: ${phone}%0A` +
    `Behandlung: ${service}%0A` +
    `Datum: ${date}%0A` +
    `Uhrzeit: ${time}`;
