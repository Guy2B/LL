...
document.querySelectorAll(".video-wrapper").forEach(wrapper => {
  ...
  // Hover IN → preview play
  wrapper.addEventListener("mouseenter", () => {
    if (!video) return;

    // Only play if not already user-started
    if (!video.classList.contains("user-playing")) {
  ...
  // Hover OUT → reset ONLY preview videos
  wrapper.addEventListener("mouseleave", () => {
    if (!video) return;

    // If it's not a clicked (active) video → reset
    if (!video.classList.contains("user-playing")) {
      video.pause();
      video.currentTime = 0;
